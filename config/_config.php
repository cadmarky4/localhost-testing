...
function get_DB_CONFIG__From_ENV_File(string $ENV_File):mixed {
	...
	$DB_CONFIG_arr["DB_Host"] = getenv("MYSQL_HOST") ?: ($_ENV["MYSQL_HOST"] ?? "localhost");
	$DB_CONFIG_arr["DB_Name"] = getenv("MYSQL_DATABASE") ?: ($_ENV["MYSQL_DATABASE"] ?? "regmondb");
	$DB_CONFIG_arr["DB_User"] = getenv("MYSQL_USER") ?: ($_ENV["MYSQL_USER"] ?? "dbuser");
	
	// Try password file first, then direct password
	$file_path = getenv("MYSQL_PASSWORD_FILE") ?: ($_ENV["MYSQL_PASSWORD_FILE"] ?? "");
	$file_contents = "";
	...
	if ($file_path && file_exists($file_path)) {
		...
		if ($file_contents !== false) {
			...
			$DB_CONFIG_arr["DB_Pass"] = "";
		}
	} else {
		// Get direct password from environment
		$direct_password = getenv("MYSQL_PASSWORD") ?: ($_ENV["MYSQL_PASSWORD"] ?? "");
		$DB_CONFIG_arr["DB_Pass"] = $direct_password;
	}
