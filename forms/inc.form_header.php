...
if ($ADMIN || $LOCATION_ADMIN || $GROUP_ADMIN || $GROUP_ADMIN_2) {
        // ADMINS: Show all users they have permission to see
        $Athletes_2_Group = array();
        $GroupsNames = array();
        $Athletes_Select = '';
        
        if ($ADMIN) {
                // Super admin sees ALL users across all groups
                $rows = $db->fetch("SELECT u2g.group_id, g.name AS group_name, u.id, u.uname, u.lastname, u.firstname
                        FROM users u
                        LEFT JOIN users2groups u2g ON u.id = u2g.user_id
                        LEFT JOIN `groups` g ON g.id = u2g.group_id
                        WHERE u.status = 1 AND u2g.status = 1
                        ORDER BY u2g.group_id, u.level DESC, u.firstname, u.lastname, u.id");
        }
        elseif ($LOCATION_ADMIN) {
                // Location admin sees users in their location
                $rows = $db->fetch("SELECT u2g.group_id, g.name AS group_name, u.id, u.uname, u.lastname, u.firstname
                        FROM users u
                        LEFT JOIN users2groups u2g ON u.id = u2g.user_id
                        LEFT JOIN `groups` g ON g.id = u2g.group_id
                        WHERE u.status = 1 AND u2g.status = 1 AND g.location_id = ?
                        ORDER BY u2g.group_id, u.level DESC, u.firstname, u.lastname, u.id", array($LOCATION));
        }
        else {
                // Group admin sees users in their group(s)
                $rows = $db->fetch("SELECT u2g.group_id, g.name AS group_name, u.id, u.uname, u.lastname, u.firstname
                        FROM users u
                        LEFT JOIN users2groups u2g ON u.id = u2g.user_id
                        LEFT JOIN `groups` g ON g.id = u2g.group_id
                        WHERE u.status = 1 AND u2g.status = 1 AND u2g.group_id = ?
                        ORDER BY u2g.group_id, u.level DESC, u.firstname, u.lastname, u.id", array($GROUP));
        }
...
elseif ($TRAINER) {
        // EXISTING TRAINER CODE - Keep as is
        $Athletes_2_Group = array();
        $GroupsNames = array();
        $Athletes_Select = '';
...
// For regular users, keep the default single option (already set above)
//####################################
?>
